...
/// daemon's original single user, minted by migration `0123_principals`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// plaintext `secret` is kept alongside (migration `0126`, `None` on older
...
///
/// Reuse is derived from the pin (migration `0127`): an **unpinned** invite
