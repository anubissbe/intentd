...
Resolve the forge from this workspace's `git remote get-url origin`. GitHub and multiple GitLab servers can be connected at the same time; never choose a forge from a global settings toggle or reuse a GitHub URL for a GitLab repository.

Prefer the provider-neutral workspace API for writes: `ws.pr.create({ title, body?, sourceBranch, targetBranch, draft?, repo? })`, `ws.pr.comment(number, body, { repo? })`, `ws.pr.review(number, "approve" | "request-changes" | "comment", body?, { repo? })`, and `ws.pr.updateBranch(number, { repo? })`. These resolve credentials on the daemon for the workspace's origin; the optional `repo` is a nested `group/subgroup/project` on that same server. They never reveal a token to the agent. `ws.pr.snapshot(number, { repo? })` reads status and merge requirements; `ws.pr.monitor` watches changes. To merge only when the user authorized it, use `ws.pr.merge(number, { expectedHeadSha, mergeMethod?, repo? })` with the head SHA you reviewed. If the head changes, review the new diff first.

For CLI-only actions on GitHub, use `gh pr` and `gh issue`. On GitLab use `glab mr` and `glab issue`, with an explicit full project URL in `--repo` for another project; use `glab api --hostname <origin-host>` for API calls. Never send a GitLab request through `gh`, fall back to github.com, or copy a PAT into a command. A saved Intent PAT enables the native API and scoped Git helper; it does not automatically sign the separate `glab` CLI in. If CLI authentication is absent, use the workspace API or have the user connect the correct server with `glab auth login --hostname <origin-host>`.

On GitLab, updating a branch rebases it on the target branch. The service waits for completion; a still-running operation or failure is reported explicitly. Rebase merge requires the project's fast-forward policy and can require a fresh review after the rebase changes the head; never change project policy just to merge. A review verdict and its summary comment are separate GitLab operations. If the verdict succeeds but the comment cannot be confirmed, inspect the MR before retrying only the missing comment. Request changes and merge trains remain subject to the server's version, tier and permissions.

Use the plain `git` CLI for status, staging, diffs, and merge checks. `ws.git.commit(message, { files?, userRequested? })` is the attributed commit helper: it auto-stages only your own changes and honors the workspace auto-commit policy (`userRequested: true` confirms an explicit user request when auto-commit is off). Intent's credential helper scopes HTTPS tokens to the repository and connected server; do not broaden that scope.
