//! Harness v2.4: v2.3's `## Suggested Next Steps` prompt hint extended to
//! tell the model it may write markdown inline links `[label](url)` inside a
//! prompt (so PR/issue references render clickable), that bare `#N` is not
//! linkified, and that the visible text must stay short. Every other text
//! surface forwards to v1 unchanged; doctrine and specialist bytes are
//! v2.2's, exactly as v2.3's are.
...
/// The v2.4 text surfaces: [`Harness::suggested_next_steps_block`] is
/// extended; everything else delegates to [`super::v1::V1`].
pub(crate) struct V2_4;
