...
//! a `role: user` row of `agent.appendMessage` (string content, or the
//! first `text` block of a block array — a text-less array gains a leading
//! text block; other roles byte-identical), and the caller-supplied
//! `agentInstructions` / `taskText` of `agent.delegate` — the task-note
//! fallback is note content and is not annotated) is persisted — and
//! delivered to the model — with the daemon-prepended single-line paragraph
//! `Message from @{login} ({displayName}), a collaborator (guest) of this
//! workspace — not the workspace owner.` plus a blank line above the
