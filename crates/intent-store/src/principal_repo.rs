...
//! (multiplayer w1, migration `0123_principals`). Principals are people
...
/// reusable and stays open across redemptions (migration `0127`).
...
pub struct WorkspaceAuthorFallback {
    ...
    /// workspace created after migration `0123`.
...
impl Store {
    ...
    /// already has one (exactly one owner per workspace, migration `0124`)
...
/// (`workspace_member_owner_uq`, migration `0124`) — the `(workspace_id,
