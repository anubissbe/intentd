...
// ─── principals / membership / credentials (migration 0123) ────────────────
...
/// Migration 0123 applies cleanly on an existing, populated database: after
...
async fn principals_migration_backfills_existing_workspaces() {
    ...
    let ws_b = WorkspaceId::from("ws-mig-b");
    {
        let store = Store::open(&tmp.path).await.expect("open store");
        for sql in [
            "DELETE FROM _sqlx_migrations WHERE version IN (123, 124)",
    ...
    let store = Store::open(&tmp.path).await.expect("reopen applies 0123");
...
/// (migration `0124`): promoting a second member or adding a second owner
...
/// (intent-hq/intentd#1868). Under the one-owner index (migration `0124`)
...
/// Migration `0124` applies on a database that already holds several owner
...
async fn one_owner_migration_repairs_duplicate_owners_before_indexing() {
    ...
    // Rewind to the pre-0124 shape and seed the duplicates the index forbids.
    ...
    sqlx::raw_sql(include_str!(
        "../migrations/0124_one_owner_per_workspace.sql"
    ))
    .execute(store.write_pool())
    .await
    .expect("0124 repairs the duplicates instead of aborting");
...
/// Migration 0126 keeps the invite link secret next to its hash: a minted
...
async fn workspace_invite_secret_round_trips_and_legacy_rows_read_none() {
    ...
    // A pre-0126 row: inserted without the `secret` column, exactly the shape
    ...
    let legacy = store
        ...
        .expect("get legacy")
        .expect("present");
    assert_eq!(legacy.secret, None, "a pre-0126 row has no stored secret");
...
/// An unpinned invite redeemed BEFORE migration 0127 was single-use when its
...
/// table, so seed the pre-0127 shapes and re-execute the migration's embedded
...
async fn invite_reusable_migration_keeps_pre_upgrade_redeemed_links_exhausted() {
    ...
    let migration = crate::MIGRATOR
        .migrations
        .iter()
        .find(|m| m.version == 127)
        .expect("migration 0127 present");
